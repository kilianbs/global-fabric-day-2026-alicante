...
print(f"🌍 Entorno: {FABRIC_ENV} | 🏠 Workspace ID: {WORKSPACE_ID} | 🔑 Lakehouse ID: {LAKEHOUSE_ID} | Tabla destino: {RAW_TABLE_NAME}")
...
abfss_path = f"abfss://{WORKSPACE_ID}@onelake.dfs.fabric.microsoft.com/{LAKEHOUSE_ID}/Tables/dbo/{RAW_TABLE_NAME}"
print(abfss_path)
...
df_enriched.write.format("delta").mode("overwrite").option("overwriteSchema","true").save(abfss_path)
print(f"[{FABRIC_ENV}] ✅ Tabla '{RAW_TABLE_NAME}' escrita en {abfss_path}")
